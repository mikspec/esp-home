...
The system SHALL automatically close the N-G bond relay when shore power is absent and inverter output phase is present, and SHALL open it in all other conditions. A 5-second stabilisation delay SHALL be observed before closing the relay to allow the inverter AC output to stabilise.
...
- **THEN** the N-G bond relay SHALL be closed after a 5-second stabilisation delay

#### Scenario: Inverter output becomes active — stabilisation delay
- **WHEN** the inverter output binary sensor transitions to ON and ng_bond_auto is ON and shore power is absent
- **THEN** the system SHALL wait 5 seconds before closing the N-G bond relay
- **AND** if conditions are no longer met at the end of the delay the relay SHALL NOT be closed
